...
internal static class DecorateServiceCollectionExtensions
{
	...
	public static IServiceCollection Decorate<TInterface>(this IServiceCollection services,Func<IServiceProvider,TInterface,TInterface> factory)
	where TInterface : class
	{
		// grab the existing registration
		var wrappedDescriptor = services.FirstOrDefault(s => s.ServiceType==typeof(TInterface));
		...
		services.Replace(ServiceDescriptor.Describe(
			typeof(TInterface),
			sp => factory(sp,(TInterface)sp.CreateInstance(wrappedDescriptor)),
			wrappedDescriptor.Lifetime)
			);
